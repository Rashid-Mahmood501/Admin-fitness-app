...
export default function MealPlanEditForm({ mealPlan, onSave, onCancel }: MealPlanEditFormProps) {
  const [selectedDay, setSelectedDay] = useState(1);
  const [selectedMealType, setSelectedMealType] = useState<string>("breakfast");
  const [editingMealId, setEditingMealId] = useState<string | null>(null);
  const [daysData, setDaysData] = useState(mealPlan.days);
  
  const { register, handleSubmit, reset, watch, setValue } = useForm<MealOption>();
  ...
  const getMealOptionsByType = (mealType: string) => {
    ...
    return currentDayData.mealOptions.filter(option => option.mealType === mealType);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-8 border border-gray-200">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-[#171616] mb-2">{mealPlan.title}</h2>
        ...
        <div className="flex space-x-4 overflow-x-auto py-6 scrollbar-hide mb-6">
          {[1, 2, 3, 4, 5, 6, 7].map((day) => {
            ...
            const hasMeals = dayData?.mealOptions.length > 0;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-700 mb-4">
              Current {selectedMealType.charAt(0).toUpperCase() + selectedMealType.slice(1)} Meals
            </h3>
              ...
              {getMealOptionsByType(selectedMealType).map((meal) => (
                <div key={meal.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-semibold text-gray-800">{meal.foodName}</h4>
          ...
          <div>
            ...
            <form onSubmit={handleSubmit(editingMealId ? handleUpdateMeal : handleAddMeal)} className="space-y-4">
              <div>
